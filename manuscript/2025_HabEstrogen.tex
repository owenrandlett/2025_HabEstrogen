...
To test if \emph{gper1} is required for the effects of estradiol on habituation, we generated larvae from \emph{gper1\textsuperscript{uab102}} heterozygous or homo/hererozygous crosses to generate clutches of larvae of mixed genotypes. 
...
While the  responsiveness distributions did not show significant differences (\autoref{fig:5ii-iv}), the cumulative mean difference plots consistently showed positive deviations, which we interpret as increased habituation (\autoref{fig:5vi}).
...
In these experiments we tested a comprehensive catalog of ER mutants in an attempt to identify which ER mediates the positive effects of estradiol on habituation learning, where estradiol-treated larvae exhibit stronger suppression of their response do DF stimuli (\autoref{fig:1}, \citealp{Lamire2023-he}). Our data lead to the following conclusions:

\subsection{\textbf{1)} The promotion of habituation learning by estradiol is mediated by an unidentified receptor.} 
Gper1, ER1, ER2a and ER2b do not mediate the positive effects of estradiol on habituation learning, as mutants (or combinations of mutants) do not show suppressed habituation learning after treatment with estradiol. Therefore, we conclude that there must be an alternative receptor or pathway that mediates the effects of estradiol on habituation learning.

As this is fundamentally a negative result, it is difficult to conclusively demonstrate this model. One major question could relate to the true nature of the mutant alleles that we have used. 
...
However, it is always possible that residual activity could still remain, perhaps via alternate splicing or alternate start codons. 
While this is a clear limitation of our approach, this explanation is very unlikely for the \emph{esr1\textsuperscript{uab118}} and \emph{esr2b\textsuperscript{uab127}} alleles, which both exhibit a lack of estradiol responsiveness in other tissues \citep{Romano2017-ep}, indicating a non-functional receptor, and  \emph{esr2b\textsuperscript{uab127}} mutants are female sterile (D. Gorelick, personal communication).
Similarly, \emph{gper1\textsuperscript{uab102}} mutants show a lack of estradiol responsiveness in heart beating rate \citep{Romano2017-ep}, but interestingly this was only observed in maternal-zygotic mutants. 
While it is difficult to imagine that enough maternal mRNA/protein could persist to mediate the effects of Gper1 in our experiments, we can not rule this out entirely. While the \emph{esr2a\textsuperscript{uab134}} mutants have no previously published phenotype, we argue that the best evidence we have against the "residual activity" hypothesis is that in our experiments the \emph{esr2a\textsuperscript{uab134}} (as well as the  \emph{esr1\textsuperscript{uab118}} and \emph{gper1\textsuperscript{uab102}} do have a phenotype, it is just of the opposite sign (see below). 

\subsection{\textbf{2)} The canonical ERs Gper1, ER1 and ER2a act to suppress habituation learning.}
...
Estradiol’s rapid effects, occurring within seconds to minutes, suggest the involvement of yet-uncharacterized membrane-bound proteins beyond GPER1, ERα, and ERβ. One hypothesis posits the existence of a “Gq-mER” (Gq-coupled membrane estrogen receptor) in select neuronal populations, which rapidly mobilizes calcium or activates kinases such as PKC and ERK1/2 upon estradiol binding. Another possibility is that estradiol directly modulates orphan GPCRs (e.g., GPRC6A) or transient receptor potential (TRP) channels, triggering downstream signaling cascades (e.g., MAPK or PI3K/Akt) that do not require the classical estrogen receptors. Additionally, estradiol may transiently bind to receptor tyrosine kinases (such as the insulin-like growth factor-1 receptor, IGF-1R), leading to rapid transactivation of those receptors via conformational changes and subsequent phosphorylation events. These non-genomic interactions, often involving scaffolding or adaptor proteins in lipid rafts/caveolae, offer plausible mechanisms to explain estradiol’s fast cellular actions while circumventing classical receptor pathways.
